...
export default function LandmarkOverlay({ landmarks, width, height }) {
  ...
  useEffect(() => {
    ...
    const is468 = landmarks.length > 68;
    ...
    if (is468) {
      ...
      for (const [i, j] of CONNECTIONS_468) {
        const a = landmarks[i];
        const b = landmarks[j];
        if (!a || !b) continue;
        ctx.beginPath();
    ...
    const count = is468 ? landmarks.length : 68;
    for (let i = 0; i < count; i++) {
      const pt = landmarks[i];
      if (!pt) continue;
      ctx.beginPath();
      ctx.arc(pt.x * width, pt.y * height, is468 ? 0.8 : 1.2, 0, Math.PI * 2);
      ctx.fill();
    }
